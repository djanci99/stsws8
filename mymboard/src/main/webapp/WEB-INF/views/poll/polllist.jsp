...
<%@page import="com.mirhenge.jyl.poll.model.JYLPoll"%>
<%@page import="com.mirhenge.jyl.poll.help.DateUtil"%>
...
<%
List<JYLPoll> plists=new ArrayList<JYLPoll>();
Object Oflist=request.getAttribute("plists");
if(Oflist!=null){
	plists=(List<JYLPoll>)Oflist;
}
%>
...
<col width="50"/><col width="100"/>
   ...
   {
	   JYLPoll poll = plists.get(i);
      %>
      <tr bgcolor='#aabbcc'>
         <td><%=(i+1) %></td>
         <td><%=poll.getId()%></td>
         <td><%=poll.getQuestion()%></td>
         <td><%=CalendarUtil.toStrDate(poll.getSdate())  %></td>
         <td><%=DateUtil.pollState(poll.getEdate())%><%=CalendarUtil.toStrDate(poll.getEdate()) %></td>
         <td><%=poll.getItemcount() %></td>
         <td><%=poll.getPolltotal() %></td>
         <td><%=CalendarUtil.toStrDate(poll.getRegdate())  %></td>
      </tr>
      <%
   }
   %>
