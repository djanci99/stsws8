...
<%!//데클러레이션 : 메소드 선언
	...
	public String color(){
		return Integer.toHexString((int)((Math.random()*15 +1)*(Math.random()*15 +1)));
	}
	...
	public String makeTable(int year, int month, int day, List<SpfpDiary> lcdtos) {
		...
		for (SpfpDiary lcd : lcdtos) { //향상된 for
			if (spfpFormat.format(lcd.getWdate()).substring(0, 8).equals(dates)) {
				...
				s += "<font  style='font-size:8px;' "; //글씨작게해서15자 들어가게끔
				s += " class=";
				s += "'team"+lcd.getTeam()+"'"; 
				s += ">";
				s += dot3(lcd.getTeam()+"조 "+lcd.getId() + "의 일지");
				s += "</font>";
				s += "</a>";
				s += "</td>";
				s += "</tr>";
			...
			<%
				...
				for (int i = 1; i <= lastDayOfMonth; i++) {
					if ((i + dayOfWeek - 1) % 7 == 0) {
						%>
						<td class="satday"><%=callist(year, month, i)%>&nbsp; <%=makeTable(year, month, i, list)%></td>
						<%
					} else if ((i + dayOfWeek - 1) % 7 == 1) {
						%>
						<td class="sunday"><%=callist(year, month, i)%>&nbsp; <%=makeTable(year, month, i, list)%></td>
						<%
					} else {
						%>
						<td class="otherday"><%=callist(year, month, i)%>&nbsp; <%=makeTable(year, month, i, list)%></td>
						<%
					}
